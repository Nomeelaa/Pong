...
{
    //paddle start in middle of the table
    init_paddle(&pong->player1, PADDLE_X_OFFSET , TABLE_HEIGHT/2 - PADDLE_HEIGHT/2, PADDLE_WIDTH, PADDLE_HEIGHT);
    init_paddle(&pong->player2, TABLE_WIDTH - PADDLE_X_OFFSET - PADDLE_WIDTH , TABLE_HEIGHT/2 - PADDLE_HEIGHT/2, PADDLE_WIDTH, PADDLE_HEIGHT);
    
    /*Paddle start in default positions
...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        if (pong->ball.x < (TABLE_WIDTH/2) + 60)
        {
            if (pong->ball.y < pong->player1.y)
            {
                pong->player1.vy = -PADDLE_SPEED;
            }
            else if (pong->ball.y > pong->player1.y)
            {
                pong->player1.vy = PADDLE_SPEED;
            }
...
void draw_table(struct Pong pong, struct Fonts fonts)
{
int mid_line_y = 0;
    int mid_line_y1 = MID_LINE_HEIGHT; 
    while(mid_line_y <= (TABLE_HEIGHT - MID_LINE_HEIGHT))
    {
        al_draw_filled_rectangle(TABLE_WIDTH / 2 - MID_LINE_WIDTH / 2, mid_line_y /*y0 component*/, TABLE_WIDTH / 2 + MID_LINE_WIDTH / 2, mid_line_y1 /*y1 componet*/, al_map_rgb(255, 0, 0));
        mid_line_y += MID_LINE_HEIGHT  + 10;
        mid_line_y1 += MID_LINE_HEIGHT + 10;
    }
    al_draw_filled_rectangle(TABLE_WIDTH / 2 - MID_LINE_WIDTH / 2, mid_line_y /*y0 component*/, TABLE_WIDTH / 2 + MID_LINE_WIDTH / 2, TABLE_HEIGHT /*y1 componet*/, al_map_rgb(255, 0, 0));
...
void render_pong(struct Pong pong, struct Fonts fonts)
{
    ...
    if (pong.state == START)
    {
        al_draw_text(fonts.extra_l_font, al_map_rgb(254, 255, 255), TABLE_WIDTH / 2, TABLE_HEIGHT / 3, ALLEGRO_ALIGN_CENTER, "PONG");
        al_draw_text(fonts.large_font, al_map_rgb(254, 255, 255), TABLE_WIDTH / 2, TABLE_HEIGHT / 2 + 40, ALLEGRO_ALIGN_CENTER, "Press enter to start");
    }
    else if (pong.state == SERVE)
    {
        draw_table(pong, fonts);
